...
export async function persistDerivedModel(supabase, plan) {
  ...
  const { data: existing, error: exErr } = await supabase
    ...
    .select('id, version, is_current, status')
    .eq('project_id', projectId)
    .order('version', { ascending: false });
  ...
  // 2. Clear the current flag so the new draft can become current (the
  //    one-current-per-project index allows only one is_current row).
  //    IMPORTANT (F3A): an APPROVED model is preserved as the latest approved
  //    record — it is NOT downgraded to 'superseded' here, so gated MVHR keeps
  //    reading it until the new draft is explicitly approved. Only unapproved
  //    working drafts (draft/needs_review) are marked superseded when replaced.
  const current = existing?.find(m => m.is_current);
  if (current) {
    const replacedByDraft = current.status === 'draft' || current.status === 'needs_review';
    const patch = replacedByDraft
      ? { is_current: false, status: 'superseded', superseded_at: new Date().toISOString() }
      : { is_current: false }; // keep approved status + approved_* audit intact
  ...
  // 3b. Point a replaced draft's superseded_by at the new model (best-effort).
  if (current && (current.status === 'draft' || current.status === 'needs_review')) {
    await supabase.from('building_models').update({ superseded_by: modelId }).eq('id', current.id);
  }
